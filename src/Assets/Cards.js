  ...
  const uploadFile = async () => {
    try {
      ...
      const response = await axiosJWTuser.patch(
        `http://localhost:3000/user/tugas/${decoded.userId}/submit/${selectedTaskID}`,
        formData
      );

      const updatedResponse = await axiosJWTuser.get(
  ...
  return (
    <div>
      ...
      <div
        ...
        onClick={() => handleShow(data.tugas.id)}
      >
        <h2 className="p-4 card-title">{data.tugas.judul}</h2>
        <p className="px-4 py-2">{data.tugas.tugas_url}</p>
        <div className="px-4 py-2 card-footer">
          <p>{moment(data.tugas.dueDate).format("DD MMMM YYYY, HH:mm")}</p>
        </div>
      </div>
